...
import net.yan100.compose.ksp.addConstModifier
import net.yan100.compose.ksp.addFinalModifier
import net.yan100.compose.ksp.addOpeneModifier
import net.yan100.compose.ksp.addOverrideModifier
import net.yan100.compose.ksp.dsl.fileDsl
import net.yan100.compose.ksp.getKsAnnotationsByAnnotationClassQualifiedName
import net.yan100.compose.ksp.isAnnotationByKClass
import net.yan100.compose.ksp.isBasicType
import net.yan100.compose.ksp.kotlinpoet.Libs
import net.yan100.compose.ksp.models.DeclarationContext
import net.yan100.compose.ksp.packageNameAsString
import net.yan100.compose.ksp.qualifiedNameAsString
import net.yan100.compose.ksp.realDeclaration
import net.yan100.compose.ksp.simpleName
import net.yan100.compose.ksp.simpleNameAsString
import net.yan100.compose.ksp.simpleNameGetShortNameStr
import net.yan100.compose.ksp.useDelegate
import net.yan100.compose.ksp.useField
import net.yan100.compose.ksp.useFile
import net.yan100.compose.ksp.useGet
...
import net.yan100.compose.toSnakeCase
...
class JpaNameClassVisitor(private val listenerSpec: AnnotationSpec?) :
  ...
  private fun getColumnName(property: JpaProperty): String {
    return (property.ksPropertyDeclaration
  ...
  fun findSuperName(classDeclaration: KSClassDeclaration): String? {
    ...
    val x =
      ab.lastOrNull()?.let {
        ...
        if (anno?.name?.isBlank() == true) anno.name
        else if (anno?.value?.isNotBlank() == true) anno.value
  ...
  private fun initNonNilProperty(
    ...
    when (
      ...
      java.util.Set::class.qualifiedName,
        ->
        builder.initializer("mutableSetOf()")
        ...
        if (fieldAndAnnotations.isNotEmpty()) {
          ...
          builder.addProperty(
            PropertySpec.builder(
              internalIdName,
              RefId::class.asTypeName().copy(nullable = true),
              KModifier.PRIVATE,
              KModifier.FINAL,
            )
              .initializer("%L", null)
          ...
          builder.addProperty(
            PropertySpec.builder("id", RefId::class)
              .mutable(true)
              .addOverrideModifier()
              .addFinalModifier()
              .getter(
          ...
          builder.addFunction(
            FunSpec.builder("getId")
              ...
              .addAnnotation(idColumnAnnotation.build())
              .addModifiers(KModifier.OPEN)
              .returns(Id::class.asTypeName().copy(nullable = true))
              .build()
          )
          builder.addFunction(
            ...
            .addFunction(
              FunSpec.builder("toString")
                ...
                .addStatement(
                  ...
                  destClassName,
                  internalIdName,
                  fieldAndAnnotations.joinToString(separator = ",") {
                      (_, p),
                    ->
                    "${p.name}=$${p.name}"
                  },
  ...
  private fun generateJpaPropertyAnnotations(
    ...
    val otherAnnotations =
      k.annotations
        .filterNot { a ->
          propertyIgnoreAnnotations.any { a.isAnnotationByKClass(it) }
        }
        .filter { a ->
