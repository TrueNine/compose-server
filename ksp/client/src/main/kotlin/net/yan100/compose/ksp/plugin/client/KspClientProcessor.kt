...
class KspClientProcessor(
  ...
  override fun process(resolver: Resolver): List<KSAnnotated> {
    val apiAnnotationName =
      Libs.net.yan100.compose.meta.annotations.client.Api.qualifiedName
    val nextHandles = mutableListOf<KSAnnotated>()
    val apiClasses =
      resolver
        .getSymbolsWithAnnotation(apiAnnotationName)
      ...
      apiClasses
        .map { cls ->
          val funList =
            cls
              .getAllFunctions()
              .filter { it.isAnnotationPresent(Api::class) }
              .toList()
          cls to funList
        }
        .toList()
    ...
    val allParameterTypes =
      allApis
        .map { (_, fnList) ->
          fnList
            .map { fn -> fn.parameters.map { r -> r.type.fastResolve() } }
            .flatten()
        }
        .flatten()
    val allReturnTypes =
      allApis
        .map { (_, fnList) ->
          fnList.mapNotNull { it.returnType?.fastResolve() }
        }
        .flatten()
    ...
    val handler =
      PropertyHandler(
        resolver,
        (allParameterTypes + allReturnTypes),
        environment.logger,
      )
    ...
    val services =
      allApis
        .map { (c, e) ->
          ...
          ClientService(
            doc = c.docString.toDoc(),
            typeName = c.qualifiedNameAsString!!,
            operations = opts,
          )
        }
        .toMutableList()
    ...
    val def =
      ClientApiStubs(
        services = services,
        definitions = allTypes.toMutableList(),
      )
    ...
    if (nextHandles.isEmpty() && !generated) {
      environment.codeGenerator
        .createNewFile(
          ...
          "META-INF/compose-client",
          "${UUID.randomUUID()}-client-ts.stub",
          "json",
        )
        .use {
