...
import net.yan100.compose.ksp.annotations.*
...
class JpaNameClassVisitor : KSTopDownVisitor<ContextData, Unit>() {
  ...
  private fun reGetAllProperty(
    ...
    return currentAllProperties
      ...
      .map { p ->
        ...
        p to
          PropertySpec.builder(p.simpleName.asString(), p.type.toTypeName().copy(jpaProperty.nullable))
            .openedModifier()
            .addModifiers(KModifier.OVERRIDE)
            .also { b ->
              b.addAnnotations(generatePropertyAnnotations(p, jpaProperty))
              val comment = "## ${jpaProperty.title}\n${jpaProperty.desc}"
              b.addKdoc(comment)

              //            log.error("name = ${p.simpleName.asString()}")
              //            log.error("nullable = ${jpaProperty.nullable}")
              //            log.error("delegate = ${jpaProperty.requireDelegate}")
  ...
  private fun generateClassAnnotations(destClassName: ClassName): List<AnnotationSpec> {
    ...
    return listOf(
      ...
      AnnotationSpec.builder(net.yan100.compose.ksp.annotations.MetaGenerated::class).addMember("value = %S", "Generated by ${LocalDate.now()}").build()
    )
  }
