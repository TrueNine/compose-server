        ...
        is List<*> ->
          ...
          when {
            ...
            method.returnType.isEnum ->
              cache.getOrPut(Pair(method.returnType, result)) {
                result.asEnum(method.returnType)
              }

            method.returnType.isAnnotation ->
              cache.getOrPut(Pair(method.returnType, result)) {
                (result as KSAnnotation).asAnnotation(method.returnType)
              }
...
@Suppress("UNCHECKED_CAST")
private fun List<*>.toArray(
  method: Method,
  valueProvider: (Any) -> Any,
): Array<Any?> {
  val array: Array<Any?> =
...
private fun Any.asByte(): Byte =
  if (this is Int) this.toByte() else this as Byte

private fun Any.asShort(): Short =
  if (this is Int) this.toShort() else this as Short

private fun Any.asLong(): Long =
  if (this is Int) this.toLong() else this as Long

private fun Any.asFloat(): Float =
  if (this is Int) this.toFloat() else this as Float

private fun Any.asDouble(): Double =
  if (this is Int) this.toDouble() else this as Double
