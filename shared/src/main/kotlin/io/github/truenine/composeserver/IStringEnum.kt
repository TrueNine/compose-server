 ...
 *
 * This interface extends [IAnyEnum] to provide type-safe handling of enums that use string values as their underlying representation. It's particularly useful
 * for enums that represent textual constants, codes, or identifiers that need to be serialized as strings in JSON, stored as VARCHAR in databases, or used in
 * HTTP parameters.
 *
 * ## Design Rationale
 *
 ...
 *
 * ```kotlin
 * enum class Language(private val code: String) : IStringEnum {
 *   ENGLISH("en"),
 *   CHINESE("zh"),
 *   JAPANESE("ja");
 *
 ...
 * @since 2023-05-28
 */
interface IStringEnum : IAnyEnum {
  /**
   * The string value of this enum constant.
   *
   * This property narrows the type from [IAnyEnum.value] to ensure that string-based enums always return string values. This enables type-safe operations and
   * better IDE support.
   *
  ...
  companion object {
     ...
     */
    @JvmStatic operator fun get(v: String?): IStringEnum? = null
  }
}
