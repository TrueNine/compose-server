...
object ResourcesLocator {
  ...
  private fun initDefinedClasses() {
    if (FILE_PROTOCOL == APPLICATION_PROTOCOL) {
      rootPath = Objects.requireNonNull<URL>(CLASS_SELF.getResource("/")).toString().replace("file:/", IString.EMPTY).replace("\\", "/")
      scanFilePackages(File(rootPath))
    } else if (JAR_PROTOCOL == APPLICATION_PROTOCOL) {
      rootPath =
        CLASS_SELF.protectionDomain.codeSource.location.path
      ...
      JarFile(jarfile).use { file ->
        ...
        while (entries.hasMoreElements()) {
          ...
          if (name.endsWith(".class")) {
            val definedClassName = name.replace("\\", "/").replace(rootPath!!, IString.EMPTY).replace(".class", IString.EMPTY).replace("/", ".")
  ...
  private fun initApplicationDirectory(): File? {
    ...
    if (JAR_PROTOCOL == APPLICATION_PROTOCOL) {
      ...
      val jar = meta.replace("jar:file:/", "").split("!/".toRegex()).dropLastWhile { it.isEmpty() }.toTypedArray()[0]
  ...
  fun classpathReader(internalPath: String?): BufferedReader {
    return BufferedReader(InputStreamReader(Objects.requireNonNull<InputStream?>(classpathInputStream(internalPath))))
  }
  ...
  val executeDir: File
    /**
     * 返回执行目录，区别如下： 1. 在 IDE 内 为项目根目录 2. 在 命令行执行情况下，为当前命令行的执行路径 3. 如果在被打包成 jar 后，为当前命令行的执行路径 其原理使用 系统属性 user.dir 实现
     *
  ...
  val generateDir: File
    get() {
      val gen = File(executeDir, "/" + GENERATED_DIR_NAME_SPACE)
      gen.mkdirs()
      return gen
    }
