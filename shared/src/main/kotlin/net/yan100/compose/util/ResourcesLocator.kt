...
object ResourcesLocator {
  ...
  /**
   * 获取一个根路径文件，区别如下： 1. 处于 ide 或者 classes 环境时，和 classes 同级 2. 处于 jar 时，为 jar
   * 同级目录
   *
   * @return root dir
   */
  ...
  private fun initApplicationProtocol(): String {
    return Objects.requireNonNull(ResourcesLocator::class.java.getResource("/"))
      .protocol
  }
  ...
  private fun initApplicationDirectory(): File? {
    ...
    try {
      uri =
        Objects.requireNonNull(ResourcesLocator::class.java.getResource("/"))
          .toURI()
  ...
  fun classpathReader(internalPath: String?): BufferedReader {
    return BufferedReader(
      InputStreamReader(
        Objects.requireNonNull<InputStream?>(classpathInputStream(internalPath))
      )
    )
  }
  ...
  val executeDir: File
    /**
     * 返回执行目录，区别如下： 1. 在 IDE 内 为项目根目录 2. 在 命令行执行情况下，为当前命令行的执行路径 3. 如果在被打包成 jar
     * 后，为当前命令行的执行路径 其原理使用 系统属性 user.dir 实现
     *
