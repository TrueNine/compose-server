...
import net.yan100.compose.systemSeparator
...
class ResourceHolder(
  ...
  init {
    log.info(
      "ResourceHolder resource loader user.dir: {}",
  ...
  private val prodConfigDir: String
    get() =
      listOf(prodDir, p.configLocation, p.location)
        .filter { it.hasText() }
        .joinToString(systemSeparator)

  private val internalConfigDir: String
    get() =
      listOf("classpath:${p.configLocation}", p.location)
        .filter { it.hasText() }
        .joinToString(systemSeparator)
  ...
  fun matchConfigResources(pattern: String): List<Resource> {
    ...
    val resolver =
      PathMatchingResourcePatternResolver(FileSystemResourceLoader())
    val resultMap = mutableMapOf<String, Resource>()

    val rootUrl = resolver.getResource(classpathDir).url
    resolver
      .getResources("$classpathDir/$pattern")
      .filter { it.exists() }
      .forEach {
        val name =
          it.url.toString().removePrefix(rootUrl.toString()).removePrefix("/")
        resultMap[name] = it
      }

    val externalConfigDirAbsolutePath =
      resolver.getResource("file:$configDir").file.absolutePath
    resolver
      .getResources("file:$externalConfigDirAbsolutePath/$pattern")
      .filter { it.exists() }
      .forEach {
        val name =
          it.file.absolutePath
            .removePrefix(externalConfigDirAbsolutePath.toString())
            .replace("\\", "/")
            .removePrefix("/")
        resultMap[name] = it
      }
