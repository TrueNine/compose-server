...
class LoggerFnsTest {
  ...
  fun `test slf4j with Class parameter returns valid logger`() {
    // Given
    val clazz = LoggerFnsTest::class.java
  ...
  fun `test slf4j with KClass parameter returns valid logger`() {
    // Given
    val kClass = LoggerFnsTest::class
  ...
  fun `test slf4j function overload resolution works correctly`() {
    ...
    val classLogger = slf4j(LoggerFnsTest::class.java)
    val kClassLogger = slf4j(LoggerFnsTest::class)
    val reifiedLogger = slf4j<LoggerFnsTest>()

    // All should have the same name since they refer to the same class
    assertEquals(LoggerFnsTest::class.java.name, classLogger.name)
    assertEquals(LoggerFnsTest::class.java.name, kClassLogger.name)
    assertEquals(LoggerFnsTest::class.java.name, reifiedLogger.name)
  }

  @Test
  fun `test slf4j with reified type parameter returns valid logger`() {
    // When
    val logger = slf4j<LoggerFnsTest>()

    // Then
    assertNotNull(logger, "Logger should not be null")
    assertEquals(LoggerFnsTest::class.java.name, logger.name, "Logger name should match the reified type name")
  }

  @Test
  fun `test logger function returns valid logger`() {
    // When
    val logger = logger<LoggerFnsTest>()
  ...
  fun `test logger and slf4j reified functions return equivalent loggers`() {
    // When
    val slf4jLogger = slf4j<LoggerFnsTest>()
    val loggerLogger = logger<LoggerFnsTest>()
  ...
  fun `test SystemLogger type alias works correctly`() {
    // When
    val logger: SystemLogger = slf4j<LoggerFnsTest>()
