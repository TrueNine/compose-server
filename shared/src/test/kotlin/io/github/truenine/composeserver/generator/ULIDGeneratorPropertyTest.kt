 ...
 *
 * These tests verify universal properties that should hold across all valid inputs and executions.
 * Each test runs multiple iterations to validate properties across a range of inputs.
 */
@DisplayName("ULIDGenerator Property Tests")
class ULIDGeneratorPropertyTest {

  /**
   * **Feature: uuid-generator, Property 2: ULID format compliance**
   * **Validates: Requirements 2.3**
   *
   * Property: For any generated UUID string, it should conform to ULID format:
   * exactly 26 characters using valid Crockford Base32 encoding (0-9, A-Z excluding I, L, O, U).
   *
  ...
  fun `property 2 - ULID format compliance`() {
    ...
    for ((index, char) in ulid.withIndex()) {
      assertTrue(
        char.uppercaseChar() in validChars,
        "Character at position $index ('$char') must be valid Crockford Base32 (0-9, A-Z excluding I, L, O, U)"
      )
    }
  ...
  /**
   * **Feature: uuid-generator, Property 3: Lexicographic ordering**
   * **Validates: Requirements 3.1**
   *
   * Property: For any two UUIDs generated sequentially (UUID1 before UUID2),
   * UUID2 should be lexicographically greater than or equal to UUID1.
   *
  ...
  fun `property 3 - lexicographic ordering`() {
    ...
    assertTrue(
      ulid2 >= ulid1,
      "Later ULID ($ulid2) should be lexicographically >= earlier ULID ($ulid1)"
    )
  }

  /**
   * **Feature: uuid-generator, Property 4: Timestamp encoding in most significant bits**
   * **Validates: Requirements 3.2**
   *
   * Property: For any generated ULID, decoding the first 10 characters should yield
   * a valid timestamp that is close to the generation time (within reasonable tolerance).
   *
  ...
  fun `property 4 - timestamp encoding in most significant bits`() {
    ...
    val testGenerator = object : ULIDGenerator() {
    ...
    assertEquals(
      generationTime,
      decodedTimestamp,
      "Decoded timestamp should match generation time"
    )

    // Verify timestamp is reasonable (not in distant past or future)
    val now = System.currentTimeMillis()
    assertTrue(
      decodedTimestamp <= now + 1000,
      "Decoded timestamp should not be in the future"
    )
    assertTrue(
      decodedTimestamp >= now - 60000,
      "Decoded timestamp should not be more than 1 minute in the past"
    )
  }

  /**
   * **Feature: uuid-generator, Property 7: Millisecond precision consistency**
   * **Validates: Requirements 3.5**
   *
   * Property: For any set of UUIDs generated within the same millisecond,
   * extracting their timestamp components should yield the same millisecond value.
   *
  ...
  fun `property 7 - millisecond precision consistency`() {
    ...
    repeat(1000) {
      ulids.add(generator.nextString())
    }
      ...
      if (groupedUlids.size > 1) {
        ...
        for (ulid in groupedUlids) {
          ...
          assertEquals(
            decodedTimestamp,
            ulidTimestamp,
            "All ULIDs with same timestamp prefix should decode to same millisecond value"
          )
        }
      }
    }
    ...
    assertTrue(
      ...
      "Should have at least one unique timestamp"
    )
  }

  /**
   * **Feature: uuid-generator, Property 9: Monotonic increment within same millisecond**
   * **Validates: Requirements 4.2**
   *
   * Property: For any sequence of UUIDs generated within the same millisecond in monotonic mode,
   * each UUID should be lexicographically greater than the previous one.
   *
  ...
  fun `property 9 - monotonic increment within same millisecond`() {
    ...
    assertTrue(
      sameMillisecondGroups.isNotEmpty(),
      "Should have at least one group with multiple ULIDs in same millisecond"
    )
    ...
    for ((timestamp, groupedUlids) in sameMillisecondGroups) {
      for (i in 1 until groupedUlids.size) {
        ...
        assertTrue(
          curr > prev,
          "In monotonic mode, ULID at index $i ($curr) should be > previous ULID ($prev) within same millisecond ($timestamp)"
        )
      }
    }
  }
  ...
  fun `monotonic mode increments counter when generating in same millisecond`() {
    ...
    assertTrue(
      stats.monotonicIncrementCount > 0,
      "Should have at least some monotonic increments when generating rapidly"
    )
  }

  /**
   * Unit test for statistics tracking: Initial state
  ...
  /**
   * Unit test for statistics tracking: Generation count increment
   * **Validates: Requirements 4.5**
   */
  @Test
  ...
  /**
   * Unit test for statistics tracking: Monotonic increment counter
   * **Validates: Requirements 4.5**
   */
  @Test
  fun `statistics should track monotonic increment count in monotonic mode`() {
    ...
    assertTrue(
      stats.monotonicIncrementCount > 0,
      "Monotonic increment count should be greater than 0 when generating rapidly in monotonic mode"
    )
    assertTrue(
      stats.monotonicIncrementCount < stats.generatedCount,
      "Monotonic increment count should be less than total generated count"
    )
  }

  /**
   * Unit test for statistics tracking: Non-monotonic mode
   * **Validates: Requirements 4.5**
   */
  @Test
  fun `statistics should not increment monotonic counter in non-monotonic mode`() {
    ...
    assertEquals(
      0L,
      stats.monotonicIncrementCount,
      "Monotonic increment count should remain 0 in non-monotonic mode"
    )
  }

  /**
   * Unit test for statistics reset functionality
   * **Validates: Requirements 4.5**
   */
  @Test
  ...
  /**
   * Unit test for statistics reset: Generation continues after reset
   * **Validates: Requirements 4.5**
   */
  @Test
  ...
  /**
   * Unit test for statistics under concurrent access
   * **Validates: Requirements 4.5**
   */
  @Test
  fun `statistics should be accurate under concurrent generation`() {
    ...
    assertEquals(
      ...
      "Generated count should be $totalIds after concurrent generation"
    )
  }

  /**
   * Unit test for statistics data class properties
   * **Validates: Requirements 4.5**
   */
  @Test
  fun `Statistics data class should have correct properties`() {
    ...
    assertTrue(
      statsString.contains("generatedCount"),
      "Statistics toString should contain generatedCount"
    )
    assertTrue(
      statsString.contains("monotonicIncrementCount"),
      "Statistics toString should contain monotonicIncrementCount"
    )
  }

  /**
   * Unit test for error handling: Invalid ULID string length
  ...
  fun `toStandardUUIDFormat throws exception for invalid ULID length`() {
    ...
    val shortException = org.junit.jupiter.api.assertThrows<IllegalArgumentException> {
      generator.toStandardUUIDFormat(shortUlid)
    }
    assertTrue(
      shortException.message?.contains("ULID must be 26 characters long") == true,
      "Exception message should indicate expected length"
    )
    assertTrue(
      shortException.message?.contains("but got ${shortUlid.length}") == true,
      "Exception message should indicate actual length"
    )

    // Test too long
    val longUlid = "01AN4Z07BY79KA1307SR9X4MV3EXTRA"
    val longException = org.junit.jupiter.api.assertThrows<IllegalArgumentException> {
      generator.toStandardUUIDFormat(longUlid)
    }
    assertTrue(
      longException.message?.contains("ULID must be 26 characters long") == true,
      "Exception message should indicate expected length"
    )
    assertTrue(
      longException.message?.contains("but got ${longUlid.length}") == true,
      "Exception message should indicate actual length"
    )

    // Test empty string
    val emptyException = org.junit.jupiter.api.assertThrows<IllegalArgumentException> {
      generator.toStandardUUIDFormat("")
    }
    assertTrue(
  ...
  /**
   * Unit test for error handling: Invalid characters in ULID
   * **Validates: Requirements 4.4**
   */
  @Test
  fun `toStandardUUIDFormat throws exception for invalid characters`() {
    ...
    for (invalidUlid in invalidChars) {
      val exception = org.junit.jupiter.api.assertThrows<IllegalArgumentException> {
        generator.toStandardUUIDFormat(invalidUlid)
      }
      assertTrue(
        exception.message?.contains("Invalid ULID character") == true,
        "Exception message should indicate invalid character for: $invalidUlid"
      )
    }
  }

  /**
   * Unit test for error handling: Verify meaningful exception messages
   * **Validates: Requirements 4.4**
   */
  @Test
  fun `error messages are meaningful and helpful`() {
    ...
    val lengthException = org.junit.jupiter.api.assertThrows<IllegalArgumentException> {
      generator.toStandardUUIDFormat("SHORT")
    }
    assertTrue(
      lengthException.message?.contains("26 characters") == true,
      "Length error should mention expected length"
    )
    assertTrue(
      lengthException.message?.contains("but got") == true,
      "Length error should mention actual length"
    )

    // Test invalid character error message
    val charException = org.junit.jupiter.api.assertThrows<IllegalArgumentException> {
      generator.toStandardUUIDFormat("01AN4Z07BY79KA1307SR9X4M@3")
    }
    assertTrue(
      charException.message?.contains("Invalid ULID character") == true,
      "Character error should mention invalid character"
    )
    assertTrue(
      charException.message?.contains("@") == true,
      "Character error should show the invalid character"
    )
  }
  ...
  fun `toStandardUUIDFormat handles case-insensitive input`() {
    ...
    assertEquals(
      uuidFromUppercase,
      uuidFromLowercase,
      "Case-insensitive decoding should produce same UUID"
    )
  }
  ...
  fun `toStandardUUIDFormat handles common character confusions`() {
    ...
    assertTrue(
      uuidWithI.matches(Regex("^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$")),
      "ULID with 'I' should be decoded as '1'"
    )
    ...
    assertTrue(
      uuidWithL.matches(Regex("^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$")),
      "ULID with 'L' should be decoded as '1'"
    )
    ...
    assertTrue(
      uuidWithO.matches(Regex("^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$")),
      "ULID with 'O' should be decoded as '0'"
    )
  }

  /**
   * **Feature: uuid-generator, Property 5: Standard UUID format conversion**
   * **Validates: Requirements 3.3**
   *
   * Property: For any generated ULID string, converting it to standard UUID format
   * should produce a valid 36-character string with hyphens at positions 8, 13, 18, and 23.
   *
   * This test verifies that:
  ...
  fun `property 5 - standard UUID format conversion`() {
    ...
    val hexPattern = Regex("^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$")
    assertTrue(
      hexPattern.matches(uuid),
      "UUID must match standard format: xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx (lowercase hex)"
    )
  }

  /**
   * **Feature: uuid-generator, Property 6: Timestamp round-trip consistency**
   * **Validates: Requirements 3.4**
   *
   * Property: For any timestamp value, encoding it into a ULID and then extracting
   * the timestamp component should yield the same value (within millisecond precision).
   *
   * This test verifies that:
  ...
  fun `property 6 - timestamp round-trip consistency`() {
    ...
    assertEquals(
      originalTimestamp,
      extractedTimestamp,
      "Extracted timestamp should match original timestamp (round-trip consistency)"
    )
  }

  /**
   * **Feature: uuid-generator, Property 8: Concurrent uniqueness**
   * **Validates: Requirements 4.1**
   *
   * Property: For any concurrent generation of N UUIDs from multiple threads,
   * all N UUIDs should be unique (no duplicates).
   *
   * This test verifies that:
  ...
  fun `property 8 - concurrent uniqueness`() {
    ...
    assertEquals(
      totalIds,
      generatedIds.size,
      "All $totalIds concurrently generated ULIDs should be unique (no duplicates)"
    )
    ...
    for (ulid in generatedIds) {
      ...
      assertTrue(
        ulid.all { it.isUpperCase() || it.isDigit() },
        "All generated ULIDs should be uppercase"
      )
    }
  }

  /**
   * Unit test for thread safety: Concurrent generation from multiple threads
  ...
  fun `concurrent generation from multiple threads produces unique ULIDs`() {
    ...
    assertEquals(
      totalIds,
      generatedIds.size,
      "Should generate exactly $totalIds unique ULIDs from $threadCount threads"
    )
  }

  /**
   * Unit test for thread safety: No race conditions in state management
   * **Validates: Requirements 2.5, 4.1**
   */
  @Test
  fun `no race conditions in timestamp and random bytes state management`() {
    ...
    assertEquals(
      totalIds,
      generatedIds.size,
      "No race conditions should occur - all ULIDs should be unique"
    )
    ...
    for (ulid in generatedIds) {
      ...
      val validChars = "0123456789ABCDEFGHJKMNPQRSTVWXYZ".toSet()
      assertTrue(
        ulid.all { it.uppercaseChar() in validChars },
        "All ULIDs should contain only valid Crockford Base32 characters"
      )
    }
  }

  /**
   * Unit test for thread safety: Statistics consistency under concurrency
   * **Validates: Requirements 2.5, 4.1**
   */
  @Test
  fun `statistics remain consistent under concurrent access`() {
    ...
    assertEquals(
      totalIds.toLong(),
      stats.generatedCount,
      "Generated count should be exactly $totalIds under concurrent access"
    )

    // Verify monotonic increment count is reasonable
    assertTrue(
      stats.monotonicIncrementCount >= 0,
      "Monotonic increment count should be non-negative"
    )
    assertTrue(
      stats.monotonicIncrementCount < stats.generatedCount,
      "Monotonic increment count should be less than total generated count"
  ...
  /**
   * Unit test for thread safety: Monotonic mode under high concurrency
   * **Validates: Requirements 2.5, 4.1**
   */
  @Test
    ...
    for ((timestamp, ulids) in timestampGroups) {
      if (ulids.size > 1) {
        ...
        assertEquals(
          ulids.size,
          ulids.toSet().size,
          "All ULIDs with timestamp $timestamp should be unique"
        )
      }
    }
  }
  ...
  /**
   * Unit test for thread safety: Secure random mode under concurrency
   * **Validates: Requirements 2.5, 4.1**
   */
  @Test
  fun `secure random mode works correctly under concurrent access`() {
    ...
    assertEquals(
      totalIds,
      generatedIds.size,
      "SecureRandom mode should produce $totalIds unique ULIDs under concurrent access"
    )
  }

  /**
   * Unit test for thread safety: Mixed mode operations
   * **Validates: Requirements 2.5, 4.1**
   */
  @Test
  fun `multiple generators can operate concurrently without interference`() {
    ...
    repeat(threadCount) { threadIndex ->
      executor.submit {
        try {
          val generator = when (threadIndex % 3) {
            0 -> generator1
            1 -> generator2
            else -> generator3
          }
    ...
    assertEquals(
      expectedTotal,
      allIds.size,
      "All ULIDs from multiple generators should be unique"
    )
  }

  /**
   * Helper function to decode timestamp from ULID timestamp component.
   * Decodes the first 10 characters of a ULID to extract the timestamp.
   */
