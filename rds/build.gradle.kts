...
val postgresqlJar by tasks.creating(Jar::class) {
    archiveClassifier.set("postgresql")
    from(postgresqlSourceSet.output, sourceSets["main"].output)
}

val mysqlSourceSet: SourceSet by sourceSets.creating {
    resources.srcDir("src/main/resources/mysql")
}
val mysqlJar by tasks.creating(Jar::class) {
...
publishing {
    repositories {
        maven(url = uri(if (version.toString().uppercase().contains("SNAPSHOT")) Repos.yunXiaoSnapshot else Repos.yunXiaoRelese)) {
            credentials {
