...
interface IAuditRepository<T : IEntity> : IBaseRepository<T> {
   ...
   * ## 根据 id 查询创建时间与当前时间间隔
   */
  fun findCrdBetweenNowDurationById(id: Id): Duration = findDurationByIdAndCrdBetween(id, datetime.now())
  fun findDurationByIdAndCrdBetween(id: Id, dt: datetime = datetime.now()): Duration = dt - findCrdById(id)

  /**
   * ## 根据 id 查询修改时间与当前时间间隔
   */
  fun findMrdBetweenNowDurationById(id: Id): Duration? = findDurationByIdAndMrdBetween(id, datetime.now())
  fun findDurationByIdAndMrdBetween(id: Id, dt: datetime = datetime.now()): Duration? = findMrdById(id)?.let { dt - it }
