...
class LazyAddressServiceImplTest {
  ...
  fun `traverseChildrenRecursive 正常递归遍历所有子节点`() {
    ...
    every { chstApi.homePage().body } returns
      "<html><body><tr class='provincetr'><td><a href='./110000.html'>北京市</a></td></tr></body></html>"
    ...
    every { chstApi.getCountyPage(any(), any()) } returns
      mockk(relaxed = true) {
        every { body } returns
          "<html><body><tr class='countytr'><td>110101</td><td>东城区</td></tr></body></html>"
      }
    // 遍历
    val visited = mutableListOf<Pair<String, Int>>()
    service.traverseChildrenRecursive("000000000000", 3, "2023") {
      children,
      depth,
      parent ->
  ...
  fun `traverseChildrenRecursive 只遍历一层`() {
    ...
    val visited = mutableListOf<String>()
    service.traverseChildrenRecursive("000000000000", 1, "2023") {
      children,
      depth,
      parent ->
      children.forEach { district -> visited += district.code.code }
  ...
  fun `traverseChildrenRecursive 空数据和无效parentCode`() {
    ...
    val visited = mutableListOf<String>()
    service.traverseChildrenRecursive("999999", 3, "2023") {
      children,
      depth,
      parent ->
      children.forEach { district -> visited += district.code.code }
    ...
    service.traverseChildrenRecursive("invalid", 3, "2023") {
      children,
      depth,
      parent ->
