...
class LazyAddressServiceImpl(private val chstApi: ICnNbsAddressApi) :
  ...
  override fun fetchChildren(
    ...
    return when (parentCodeObj.level) {
      ...
      else -> emptyList<ILazyAddressService.CnDistrict>().also {
  ...
  override fun fetchChildrenRecursive(
    ...
    // 或者抛出异常:
    // throw UnsupportedOperationException("Recursive search is not supported by this deprecated implementation.")
  }
      ...
      Jsoup.parse(it)
        ...
        .mapNotNull { kv ->
          ...
          if (codeElement != null && nameElement != null) {
            ...
            val expectedLength = when (className) {
              "citytr" -> 4
              "countytr" -> 6
              "towntr" -> 9
              else -> 0
            ...
            if (fullCode != null && codePart?.length == expectedLength && ILazyAddressService.verifyCode(fullCode)) {
              ...
              val leaf = when (className) {
                "citytr" -> false
                "countytr", "towntr" -> kv.child(1).select("a").isEmpty()
                else -> false
              }
