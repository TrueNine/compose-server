...
class LazyAddressServiceImpl(private val chstApi: ICnNbsAddressApi) :
  ...
  override fun fetchChildren(
    ...
    if (yearVersion != supportedDefaultYearVersion) {
      logger?.warn(
        "Unsupported yearVersion {} requested for NBS service, returning empty list.",
        yearVersion,
      )
      return emptyList()
    }
    ...
    return when (parentCodeObj.level) {
      ...
      1 ->
        findAllCityByCodeInternal(parentCode, yearVersion) // parent is province
      2 ->
        findAllCountyByCodeInternal(parentCode, yearVersion) // parent is city
      3 ->
        findAllTownByCodeInternal(parentCode, yearVersion) // parent is county
      4 ->
        findAllVillageByCodeInternal(parentCode, yearVersion) // parent is town
      else ->
        emptyList<ILazyAddressService.CnDistrict>().also {
          logger?.warn(
            "findChildren called with unsupported parent level: {} for code: {}",
            parentCodeObj.level,
            parentCode,
          )
        }
  ...
  override fun fetchDistrict(
    ...
    logger?.debug(
      "Attempting findDistrict for code: {}, starting year: {}",
      code,
      yearVersion,
    )
    // 实现版本回退逻辑
    ...
    while (currentYearVersion != null) {
      ...
      if (currentYearVersion != supportedDefaultYearVersion) {
        logger?.trace(
          "Skipping unsupported year {} for NBS service.",
          currentYearVersion,
        )
      ...
      val codeObj =
        ILazyAddressService.createCnDistrictCode(code)
          ?: return null // 无效 code 直接返回
      val parentCode =
        codeObj.back()?.code?.toString()
          ?: if (codeObj.level == 1) ILazyAddressService.DEFAULT_COUNTRY_CODE
          else null

      if (parentCode != null) {
        val children =
          fetchChildren(parentCode, currentYearVersion) // 调用本类实现的 findChildren
        val found = children.firstOrNull { it.code.code == codeObj.code }
        if (found != null) {
          logger?.debug(
            "Found district {} in year {}",
            code,
            currentYearVersion,
          )
        ...
        logger?.warn(
          "Cannot determine parent for code {} to perform lookup.",
          code,
        )
      ...
      // 如果没找到，尝试更早的版本
      logger?.trace(
        "District {} not found in year {}, trying older version.",
        code,
        currentYearVersion,
      )
      currentYearVersion = lastYearVersionOrNull(currentYearVersion)
    ...
    logger?.warn(
      "District {} not found in any supported version starting from {}.",
      code,
      yearVersion,
    )
  ...
  override fun fetchChildrenRecursive(
    ...
    logger?.warn(
      "findChildrenRecursive is not efficiently implemented in the deprecated NBS service."
    )
  ...
  private fun findAllProvincesInternal(
    yearVersion: String
  ): List<ILazyAddressService.CnDistrict> {
  ...
  private fun findAllCityByCodeInternal(
    provinceCode: String,
    yearVersion: String,
  ): List<ILazyAddressService.CnDistrict> {
    // 年份检查已在外层完成
    // 使用接口的公共方法创建 Code 对象
    val model =
      ILazyAddressService.createCnDistrictCode(provinceCode)
        ?: return emptyList()
    // 简单的层级检查
    if (model.level != 1) {
      logger?.error(
        "Internal: findAllCityByCodeInternal called with non-province code: {}",
        provinceCode,
      )
      return emptyList()
    }
  ...
  private fun findAllCountyByCodeInternal(
    cityCode: String,
    yearVersion: String,
  ): List<ILazyAddressService.CnDistrict> {
    // 年份检查已在外层完成
    val model =
      ILazyAddressService.createCnDistrictCode(cityCode) ?: return emptyList()
    if (model.level != 2) {
      logger?.error(
        "Internal: findAllCountyByCodeInternal called with non-city code: {}",
        cityCode,
      )
      return emptyList()
    }
  ...
  private fun findAllTownByCodeInternal(
    countyCode: String,
    yearVersion: String,
  ): List<ILazyAddressService.CnDistrict> {
    // 年份检查已在外层完成
    val model =
      ILazyAddressService.createCnDistrictCode(countyCode) ?: return emptyList()
    if (model.level != 3) {
      logger?.error(
        "Internal: findAllTownByCodeInternal called with non-county code: {}",
        countyCode,
      )
      return emptyList()
    }
  ...
  private fun findAllVillageByCodeInternal(
    townCode: String,
    yearVersion: String,
  ): List<ILazyAddressService.CnDistrict> {
    // 年份检查已在外层完成
    val model =
      ILazyAddressService.createCnDistrictCode(townCode) ?: return emptyList()
    if (model.level != 4) {
      logger?.error(
        "Internal: findAllVillageByCodeInternal called with non-town code: {}",
        townCode,
      )
      return emptyList()
    }
  ...
  private fun extractProvinces(
    ...
    return page?.let {
      Jsoup.parse(it)
        ...
        .mapNotNull { link ->
          val hrefCode =
            link.attr("href").removePrefix("./").removeSuffix(".html")
          ...
          if (ILazyAddressService.verifyCode(fullCode)) {
            ...
            logger?.error(
              "Failed to parse province code from href: {}",
              link.attr("href"),
            )
            null
          }
      ...
      Jsoup.parse(it)
        ...
        .mapNotNull { element ->
          ...
          if (code.length == 12 && ILazyAddressService.verifyCode(code)) {
            ...
            logger?.error(
              "Failed to parse village code: {} or name: {}",
              code,
              name,
            )
            null
      ...
      Jsoup.parse(it)
        ...
        .mapNotNull { kv ->
          ...
          if (codeElement != null && nameElement != null) {
            ...
            if (
              fullCode != null &&
                codePart?.length == expectedLength &&
                ILazyAddressService.verifyCode(fullCode)
            ) {
              ...
              logger?.error(
                "Failed to parse plain item code from href: {} or name for class: {}",
                href,
                className,
              )
              null
            ...
            // 使用接口的验证方法
            if (
              codeText.all { it.isDigit() } &&
                codeText.length == expectedLength &&
                ILazyAddressService.verifyCode(fullCode)
            ) {
              toCnDistrict(fullCode, nameText, true)
            } else {
              logger?.error(
                "Failed to parse plain item text code: {} or name: {} for class: {}",
                codeText,
                nameText,
                className,
              )
