 ...
 *
 * Provides a standard configuration for Redis test containers used in cache
 * integration tests. By implementing this interface, test classes can obtain a
 * preconfigured Redis test instance and use extension functions for convenient
 * testing.
 *
 * Important: container reuse and data cleanup
 *
 * By default, to improve test performance, all containers are reusable. This
 * means data may remain between tests.
 *
 * Data cleanup responsibility:
 * - You must clean up Redis data in tests (for example using `@BeforeEach` or
 *   `@AfterEach`).
 ...
 *
 * Usage (recommended extension style): use the `redis` extension function with
 * `resetToInitialState` to automatically clear data before tests.
 *
...
interface ICacheRedisContainer : ITestContainerBase {
  companion object {
     ...
     * - Container is started automatically when Spring properties are injected
     *
     * Important: because of container reuse, data will remain between tests,
     * so make sure to perform proper cleanup in tests.
     */
     ...
     *
     * Automatically injects Redis connection properties into the Spring test
     * environment:
     * - host
     * - port
     *
   ...
   *
   * Provides a convenient way to test with a Redis container and supports
