...
import io.github.truenine.composeserver.oss.minio.MinioClientWrapper
...
class MinioAutoConfigurationTest {
  companion object {
    private val minioContainer =
      GenericContainer("minio/minio:latest")
        ...
        .withCommand("server", "/data", "--console-address", ":9001", "--address", ":9000")
        .waitingFor(
          org.testcontainers.containers.wait.strategy.Wait.forLogMessage(".*MinIO Object Storage Server.*", 1)
    ...
    fun properties(registry: DynamicPropertyRegistry) {
      ...
      registry.add("compose.oss.minio.endpoint-port") { minioContainer.getMappedPort(9000) }
      registry.add("compose.oss.minio.access-key") { "minio" }
      registry.add("compose.oss.minio.secret-key") { "minio123" }
      registry.add("compose.oss.minio.exposed-base-url") { "http://localhost:${minioContainer.getMappedPort(9000)}" }
    }
  }
  ...
  fun `测试 Minio 连接是否正常`() {
    ...
    kotlin.test.assertTrue(wrapper.existsBucketByName(testBucketName), "创建测试 bucket 失败")

    // 清理测试数据
    val buckets = wrapper.fetchAllBucketNames()
    kotlin.test.assertTrue(buckets.contains(testBucketName), "无法在 bucket 列表中找到测试 bucket")
  }
}
