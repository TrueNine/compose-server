...
class PemFormatTest {
  ...
  fun `测试 PEM 内容格式化`() {
    ...
    pem.lines().forEach { line ->
      if (line != "-----BEGIN TEST-----" && line != "-----END TEST-----" && line.isNotBlank()) {
  ...
  fun `测试不同密钥类型`(testCase: String, keyType: String?, expectedBegin: String, expectedEnd: String) {
  ...
  @Test
  fun `测试空 Base64 内容`() {
    assertThrows<IllegalArgumentException>("Base64 内容不能为空") { PemFormat["", "TEST"] }
  }
