...
object Keys {
  ...
  fun readAesKeyByBase64(base64: String): SecretKeySpec? =
    runCatching { SecretKeySpec(IBase64.decodeToByte(base64), "AES") }.onFailure { log.error(Keys::readAesKeyByBase64.name, it) }.getOrNull()
   ...
   */
  @JvmStatic
  fun readEccPublicKeyByBase64(base64: String): PublicKey? = readPublicKeyByBase64AndAlg(base64, EncryptAlgorithmTyping.ECC)
  ...
  private fun readPublicKeyByBase64AndAlg(base64: String, alg: EncryptAlgorithmTyping): PublicKey? {
    return runCatching { X509EncodedKeySpec(IBase64.decodeToByte(base64)).run { KeyFactory.getInstance(alg.value).generatePublic(this) } }
  ...
  private fun readPrivateKeyByBase64AndAlg(base64: String, alg: EncryptAlgorithmTyping): PrivateKey? {
    return runCatching { PKCS8EncodedKeySpec(IBase64.decodeToByte(base64)).run { KeyFactory.getInstance(alg.value).generatePrivate(this) } }
   ...
   */
  @JvmStatic
  fun generateRsaKeyPair(seed: String = generateRandomAsciiString(), keySize: Int = RSA_KEY_SIZE): IRsaExtKeyPair? {
    return generateKeyPair(seed, keySize, EncryptAlgorithmTyping.RSA.value, "SunRsaSign")?.let {
   ...
   */
  @JvmStatic
  fun generateEccKeyPair(seed: String = generateRandomAsciiString()): EccExtKeyPair? {
    return runCatching {
  ...
  fun readRsaKeyPair(rsaPublicKeyBase64: String, rsaPrivateKeyBase64: String): IRsaExtKeyPair {
    return RsaExtKeyPair(readRsaPublicKeyByBase64(rsaPublicKeyBase64)!!, readRsaPrivateKeyByBase64(rsaPrivateKeyBase64)!!)
  }
