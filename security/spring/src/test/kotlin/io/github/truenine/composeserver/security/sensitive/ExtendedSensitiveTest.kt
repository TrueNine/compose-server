...
class ExtendedSensitiveTest {
  ...
  fun `test user info desensitization`() {
    ...
    val result = mockMvc.get("/test/sensitive/extended/user").andExpect {
      status { isOk() }
  ...
  fun `test simple data desensitization`() {
    ...
    val result = mockMvc.get("/test/sensitive/extended/simple").andExpect {
      status { isOk() }
      content { contentType(MediaType.APPLICATION_JSON) }
    }.andReturn()
  ...
  fun `test nested data desensitization`() {
    ...
    val result = mockMvc.get("/test/sensitive/extended/nested").andExpect {
      status { isOk() }
      content { contentType(MediaType.APPLICATION_JSON) }
    }.andReturn()
  ...
  fun `test collection desensitization`() {
    ...
    val result = mockMvc.get("/test/sensitive/extended/collection").andExpect {
      status { isOk() }
      content { contentType(MediaType.APPLICATION_JSON) }
    }.andReturn()
  ...
  fun `test array desensitization`() {
    ...
    val result = mockMvc.get("/test/sensitive/extended/array").andExpect {
      status { isOk() }
      content { contentType(MediaType.APPLICATION_JSON) }
    }.andReturn()
  ...
  fun `test map desensitization`() {
    ...
    val result = mockMvc.get("/test/sensitive/extended/map").andExpect {
      status { isOk() }
      content { contentType(MediaType.APPLICATION_JSON) }
    }.andReturn()
  ...
  fun `test no annotation should not desensitize`() {
    ...
    val result = mockMvc.get("/test/sensitive/extended/no-annotation").andExpect {
      status { isOk() }
      content { contentType(MediaType.APPLICATION_JSON) }
    }.andReturn()
