...
class ViewErrorAction : AnAction("查看错误", "查看文件或文件夹中的错误、警告信息", null) {
  ...
  override fun actionPerformed(e: AnActionEvent) {
    ...
    // 在后台任务中执行错误收集
    ProgressManager.getInstance().run(object : Task.Backgroundable(project, "正在收集错误信息...", true) {
      ...
      override fun onCancel() {
        ...
        // 在 EDT 中显示取消消息
        com.intellij.openapi.application.ApplicationManager.getApplication().invokeLater {
          Messages.showInfoMessage(project, "错误查看操作已取消", "操作取消")
        }
      }
  ...
  private fun filterErrorReport(
    ...
    return errorReport.map { fileInfo ->
