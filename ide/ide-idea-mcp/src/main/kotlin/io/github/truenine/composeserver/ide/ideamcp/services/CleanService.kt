...
class CleanServiceImpl(private val fileManager: FileManager) : CleanService {

  override suspend fun cleanCode(
    project: Project,
    virtualFile: VirtualFile,
    options: CleanOptions
   ...
   * 收集需要处理的文件
   */
  private fun collectFilesToProcess(project: Project, virtualFile: VirtualFile): List<VirtualFile> {
    return if (virtualFile.isDirectory) {
      fileManager.collectFilesRecursively(virtualFile) { file ->
        isCodeFile(file)
      }
    } else {
  ...
  private fun isCodeFile(file: VirtualFile): Boolean {
    ...
    return extension in setOf(
      "kt", "java", "scala", "groovy", "js", "ts", "jsx", "tsx",
      "py", "rb", "php", "go", "rs", "cpp", "c", "h", "hpp",
      "cs", "vb", "swift", "dart", "xml", "html", "css", "scss",
      "less", "json", "yaml", "yml", "properties", "gradle"
    )
  }
  ...
  private suspend fun processFile(
    ...
    virtualFile: VirtualFile,
    options: CleanOptions,
    operations: MutableList<CleanOperation>
