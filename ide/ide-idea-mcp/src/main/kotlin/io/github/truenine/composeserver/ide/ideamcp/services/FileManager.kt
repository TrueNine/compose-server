...
class FileManager {
   ...
   * 递归收集文件的内部实现
   */
  private fun collectFilesRecursivelyInternal(
    virtualFile: VirtualFile,
    ...
    if (virtualFile.isDirectory) {
      ...
      try {
        virtualFile.children?.forEach { child ->
          collectFilesRecursivelyInternal(child, filter, result)
        }
      } catch (e: Exception) {
  ...
  object Filters {
     ...
     * 只包含 Kotlin 文件
     */
    val kotlinFiles: (VirtualFile) -> Boolean = { file ->
      file.extension?.lowercase() == "kt"
    }

    /**
     * 只包含 Java 文件
     */
    val javaFiles: (VirtualFile) -> Boolean = { file ->
      file.extension?.lowercase() == "java"
    }

    /**
     * 排除隐藏文件和目录
     */
    val excludeHidden: (VirtualFile) -> Boolean = { file ->
     ...
     * 组合多个过滤器
     */
    fun combine(vararg filters: (VirtualFile) -> Boolean): (VirtualFile) -> Boolean = { file ->
      filters.all { it(file) }
    }
  }
