...
class CleanCodeAction : AnAction("清理代码", "使用 IDEA 功能清理和格式化代码", null) {
  ...
  override fun update(e: AnActionEvent) {
    ...
    e.presentation.isEnabledAndVisible = project != null && virtualFile != null && 
      (virtualFile.isDirectory || isCodeFile(virtualFile))
  }
  ...
  override fun actionPerformed(e: AnActionEvent) {
    ...
    // 在后台任务中执行清理操作
    ProgressManager.getInstance().run(object : Task.Backgroundable(project, "正在清理代码...", true) {
      ...
      override fun run(indicator: ProgressIndicator) {
        try {
          ...
          val result = runBlocking {
            cleanService.cleanCode(project, virtualFile, cleanOptions)
          }
      ...
      override fun onCancel() {
        ...
        // 在 EDT 中显示取消消息
        com.intellij.openapi.application.ApplicationManager.getApplication().invokeLater {
          Messages.showInfoMessage(project, "代码清理操作已取消", "操作取消")
        }
      }
      
      override fun onSuccess() {
        currentResult?.let { result ->
          McpLogManager.info("代码清理成功完成 - 总耗时: ${result.executionTime}ms", "CleanCodeAction")
        }
      }
    })
  }
  ...
  private fun isCodeFile(virtualFile: VirtualFile): Boolean {
    ...
    return extension in setOf(
      "kt", "kts", "java", "scala", "groovy", "js", "ts", "jsx", "tsx",
      "py", "rb", "php", "go", "rs", "cpp", "c", "h", "hpp", "cs",
      "xml", "html", "css", "scss", "less", "json", "yaml", "yml"
    )
  }
   ...
   * 显示详细结果对话框
   */
  private fun showDetailedResultDialog(
    project: Project, 
    result: io.github.truenine.composeserver.ide.ideamcp.services.CleanResult, 
    fileName: String
  ) {
