...
class ErrorHandlingTest : BasePlatformTestCase() {
  ...
  fun testErrorDetailsWithLongMessage() {
    // 测试长错误消息
    val longMessage = "这是一个非常长的错误消息，用于测试错误详情类是否能够正确处理长文本内容"
    val errorDetails = ErrorDetails(
      type = "LongMessageError",
      message = longMessage
    )
  ...
  fun testErrorDetailsWithMultipleSuggestions() {
    ...
    suggestions.forEach { suggestion ->
      assertTrue(errorDetails.suggestions.contains(suggestion))
    }
  }

  fun testErrorDetailsEquality() {
    // 测试错误详情相等性
    val errorDetails1 = ErrorDetails(
      type = "TestError",
      message = "测试消息",
      suggestions = listOf("建议1")
    )
    
    val errorDetails2 = ErrorDetails(
      type = "TestError",
      message = "测试消息",
      suggestions = listOf("建议1")
    )
