...
class SynchronizedSimpleSnowflakeTest {
  ...
  @BeforeEach
  fun setUp() {
    snowflake = SynchronizedSimpleSnowflake(
      workId = 1L,
      datacenterId = 1L,
      sequence = 0L,
  ...
  fun `测试构造函数参数验证 - workId为0`() {
    val exception = assertThrows<IllegalArgumentException>("workId为0应该抛出异常") {
      SynchronizedSimpleSnowflake(
        workId = 0L,
        datacenterId = 1L,
        sequence = 0L,
  ...
  fun `测试构造函数参数验证 - workId为32`() {
    val exception = assertThrows<IllegalArgumentException>("workId为32应该抛出异常") {
      SynchronizedSimpleSnowflake(
        workId = 32L,
        datacenterId = 1L,
        sequence = 0L,
  ...
  fun `测试构造函数参数验证 - workId为负数`() {
    val exception = assertThrows<IllegalArgumentException>("workId为负数应该抛出异常") {
      SynchronizedSimpleSnowflake(
        workId = -1L,
        datacenterId = 1L,
        sequence = 0L,
  ...
  fun `测试构造函数参数验证 - datacenterId为0`() {
    val exception = assertThrows<IllegalArgumentException>("datacenterId为0应该抛出异常") {
      SynchronizedSimpleSnowflake(
        workId = 1L,
        datacenterId = 0L,
        sequence = 0L,
    ...
    assertAll(
      { assertTrue(exception.message!!.contains("datacenterId"), "异常消息应该包含'datacenterId'") }
    )
  }

  @Test
  fun `测试构造函数参数验证 - datacenterId为32`() {
    val exception = assertThrows<IllegalArgumentException>("datacenterId为32应该抛出异常") {
      SynchronizedSimpleSnowflake(
        workId = 1L,
        datacenterId = 32L,
        sequence = 0L,
  ...
  fun `测试构造函数参数验证 - datacenterId为负数`() {
    val exception = assertThrows<IllegalArgumentException>("datacenterId为负数应该抛出异常") {
      SynchronizedSimpleSnowflake(
        workId = 1L,
        datacenterId = -1L,
        sequence = 0L,
  ...
  fun `测试生成ID的唯一性`() {
    ...
    assertAll(
      { assertEquals(count, ids.size, "应该生成 $count 个唯一ID") }
    )
  }
  ...
  fun `测试ID的递增性`() {
    ...
    assertAll(
      { assertTrue(id2 > id1, "后续生成的ID应该大于前一个ID") }
    )
  }
  ...
  fun `测试序列号溢出处理`() {
    // 设置初始序列号为接近最大值
    val snowflake = SynchronizedSimpleSnowflake(
      workId = 1L,
      datacenterId = 1L,
  ...
  fun `测试时钟回拨异常`() {
    // 创建一个自定义的雪花生成器实现
    val mockSnowflake = object : ISnowflakeGenerator {
    ...
    // 尝试生成第二个ID，应该抛出异常
    val exception = assertThrows<RuntimeException>("时钟回拨应该抛出异常") {
      mockSnowflake.next()
    }
    assertAll(
