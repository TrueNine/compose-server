 ...
 */
object Encryptors {
  @JvmStatic private val log = slf4j(Encryptors::class)

  @JvmStatic private val sha1 = MessageDigest.getInstance("SHA-1")
  ...
  private fun encrypt(
    ...
    runCatching {
      ...
      .onFailure { log.error(::encrypt.name, it) }
      .getOrNull()
  ...
  private fun basicDecrypt(
    ...
    runCatching {
      ...
      .onFailure { log.error(::basicDecrypt.name, it) }
      .getOrNull()
    ...
    runCatching {
        Cipher.getInstance("ECIES", "BC").run {
          ...
          doFinal(data.toByteArray(charset))
            .let(net.yan100.compose.core.encrypt.Base64Helper::encode)
        }
      }
      .onFailure { log.error(::encryptByEccPublicKey.name, it) }
      .getOrNull()
  ...
  fun decryptByEccPrivateKey(
    ...
    runCatching {
      ...
      .onFailure { log.error(::decryptByEccPrivateKey.name, it) }
      .getOrNull()
  ...
  fun encryptByAesKey(
    ...
    runCatching {
      ...
      .onFailure { log.error(::encryptByAesKey.name, it) }
      .getOrNull()
  ...
  fun decryptByAesKey(
    ...
    kotlin
      ...
      .onFailure { log.error(::decryptByAesKey.name, it) }
      .getOrNull()
