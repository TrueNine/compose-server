...
object Keys {
  ...
  fun readAesKeyByBase64(base64: String): SecretKeySpec? =
    runCatching { SecretKeySpec(Base64Helper.decodeToByte(base64), "AES") }
      .onFailure { log.error(::readAesKeyByBase64.name, it) }
      .getOrNull()
  ...
  fun writeKeyToBase64(key: Key): String? =
    runCatching { key.encoded.encodeBase64String }
      .onFailure { log.error(::writeKeyToBase64.name, it) }
      .getOrNull()

  @JvmStatic
  fun writeKeyToPem(key: Key, keyType: String? = null): String? =
    runCatching { PemFormat.ofKey(key, keyType) }
      .onFailure { log.error(::writeKeyToPem.name, it) }
      .getOrNull()
  ...
  private fun readPublicKeyByBase64AndAlg(base64: String, alg: EncryptAlgorithmTyping): PublicKey? {
    return runCatching {
        X509EncodedKeySpec(Base64Helper.decodeToByte(base64)).run {
          KeyFactory.getInstance(alg.value).generatePublic(this)
        }
      }
      .onFailure { log.error(::readPublicKeyByBase64AndAlg.name, it) }
      .getOrNull()
  }
  ...
  private fun readPrivateKeyByBase64AndAlg(
    ...
    return runCatching {
      ...
      .onFailure { log.error(::readPrivateKeyByBase64AndAlg.name, it) }
      .getOrNull()
  }
  ...
  fun generateKeyPair(
    ...
    return runCatching {
      ...
      .onFailure { log.error(::generateKeyPair.name, it) }
      .getOrNull()
  }
  ...
  fun generateEccKeyPair(seed: String = this.generateRandomAsciiString()): EccKeyPair? {
    return runCatching {
      ...
      .onFailure { log.error(::generateEccKeyPair.name, it) }
      .getOrNull()
  }
  ...
  fun generateAesKey(seed: String = DEFAULT_SEED, keySize: Int = AES_KEY_SIZE): SecretKeySpec? {
    return runCatching {
      ...
      .onFailure { log.error(::generateAesKey.name, it) }
      .getOrNull()
  }
