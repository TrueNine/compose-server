...
interface IRegexes {
  companion object {
    ...
    private const val CHINA_ID_CARD_PREFIX =
      "^${ONE_ONE}${ZERO_ZERO_ZERO_ONE}${ZERO_ONE}${YEAR}${MONTH}${DAY}\\d{3}[xX0-9]"
     ...
     * - 12位
     */
    const val CHINA_AD_CODE =
      "(?:[1-9]\\d{1}|[1-9]\\d{3}|[1-9]\\d{5}|[1-9]\\d{8}|[1-9]\\d{11})\$"
    ...
    /** 中国残疾证号 */
    const val CHINA_DIS_CARD =
      "${CHINA_ID_CARD_PREFIX}[1-7][1-4](?:[bB][1-9])?$"
    ...
    const val RBAC_NAME =
      "^([a-zA-Z]){1}|([a-zA-Z][a-zA-Z0-9_\\-:]*[a-zA-Z0-9])\$"
  }
}
