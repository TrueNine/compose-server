<template>
  <!-- 复制代码 -->
  <q-btn style="margin-block: 1rem" @click="copyCode" icon="save" v-if="code !== `` && code !== `{}`">复制响应内容</q-btn>
  <!-- 响应结果 -->
    ...
    <div class="col-5 bg-dark shadow-14">
      <q-list class="response-box">
        ...
        <q-item v-if="response.code">
          ...
          <q-item-section>
            <span></span>
            <q-chip :color="response.code > 399 ? `red` : `green`" text-color="dark">{{ props.response.code }} </q-chip>
          </q-item-section>
          <q-item-section> 耗时:</q-item-section>
          <q-item-section
            ><span>
              <q-chip :color="props.time > 5 ? `yellow` : `green`" outline>{{ props.time }} ms</q-chip>
            </span></q-item-section
          >
        </q-item>
        <q-item v-else>
            ...
            <q-expansion-item>
              <template #header>
                <q-item>
                  ...
                  <q-item-section>
                    <span>
                      <q-chip outline color="green">{{ headerSize(props.response.headers) }}</q-chip>
                    </span>
                  </q-item-section>
                </q-item>
              </template>
              <q-card class="header-card-box">
                <q-card-section>
                  <q-list>
                    <q-item clickable :key="k" v-for="(v, k) in props.response.headers">
