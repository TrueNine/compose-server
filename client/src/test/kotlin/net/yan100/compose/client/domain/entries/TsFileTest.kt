...
class TsFileTest {
  ...
  fun `SingleServiceClass 测试渲染服务方法`() {
            ...
            listOf(
              TsVal.Function(
                ...
                code(
                  """let __uri = ''
    const __method = 'GET'
    return (await this.executor({uri: __uri as unknown as `/${'$'}{string}`, method: __method})) as unknown as Promise<Abc>"""
                    .trimIndent()
                )
              },
              TsVal.Function(
                ...
                returnType =
                  TsUseVal.Return(
                    TsTypeVal.Promise(TsGeneric.Used(TsTypeVal.String))
                  ),
                params =
                  listOf(
                    TsUseVal.Parameter(
    ...
    assertEquals(
      TsName.PathName(name = "AuthApi", path = "service/a"),
      expectResult.fileName,
    )
    assertEquals(
      TsExport.ExportedDefined(name = expectResult.fileName),
      expectResult.exports.first(),
    )
  ...
  fun `SingleInterface 正确地序列化了接口类型`() {
    ...
    assertTrue {
      result.imports.any { it.fromPath.contains("net_yan100_compose") }
    }
  ...
  private fun newWriterFile(
    name: String,
    block: ((Writer) -> Unit)? = null,
  ): File {
  ...
  private val interfaces =
    TsScope.Interface(
      generics =
        listOf(
          TsGeneric.Defined(
            TsName.Name("<net.yan100.compose.AInterface::[] T1>"),
            0,
          ),
          TsGeneric.Defined(
            TsName.Name("<net.yan100.compose.AInterface::[out] T2>"),
            1,
          ),
          TsGeneric.Defined(
            TsName.Name("<net.yan100.compose.AInterface::[out] T3>"),
            2,
          ),
              ...
              TsTypeVal.Object(
                elements =
                  listOf(
                    TsUseVal.Prop(
                      name = TsName.Name("a"),
                      typeVal = TsTypeVal.Ref(typeName = TsName.Name("A")),
                    )
                  )
              ),
          ),
          TsUseVal.Prop(name = TsName.Name("acc"), typeVal = TsTypeVal.Any),
        ),
        ...
        listOf(
          TsTypeVal.Ref(
            typeName =
              TsName.PathName(
                "SuperInterface1",
                "static/net_yan100_compose_server",
              ),
            usedGenerics =
              listOf(
                TsGeneric.Used(
                  used =
                    TsTypeVal.Ref(
                      typeName =
                        TsName.PathName("Abs", "static/net_yan100_compose")
                    ),
                  index = 0,
                )
              ),
          ...
          TsTypeVal.Ref(
            typeName =
              TsName.PathName("SuperInterface2", "static/net_yan100_compose")
          ),
        ),
    )
}
