...
sealed class TsName {
  ...
  open fun redirect(supportAs: Boolean = true): TsName {
    return when (this) {
      is Name ->
        if (supportAs) As(name, "${name}${name}") else Name("${name}${name}")
   ...
   * ```
   */
  data class As(val name: String, val asName: String) : TsName() {
   ...
   * ```
   */
  data class Name(val name: String) : TsName() {
   ...
   * ```
   */
  data class Generic(val name: String) : TsName() {
    override fun toString(): String = name.unwrapGenericName()
   ...
   * @param path 导入路径
   */
  data class PathName(val name: String, val path: String = "") : TsName() {
    override fun toString(): String =
      "${if (path.isNotEmpty()) "$path/" else ""}${name}"
  }
}
