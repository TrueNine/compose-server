...
internal fun String.toTsPathName(): TsName.PathName {
  ...
  if (packageAndClassName.size == 1) {
    return TsName.PathName(name = this.toTsName().name, path = "")
  }
  ...
  return TsGeneric.Used(
    used =
      TsTypeVal.Ref(
        ...
        usedGenerics =
          this.usedGenerics.map { it.toTsGenericUsed(typeNameProvider) },
      ),
    index = index,
  )
}
    ...
    TsGeneric.Used(
      used =
        TsTypeVal.Ref(
          ...
          usedGenerics =
            used.usedGenerics.map { it.toTsGenericUsed(typeNameProvider) },
        ),
      index = index,
    )
  }
...
internal fun <T : Any> String.ifGenericName(
  default: T,
  block: (String) -> T = { default },
): T {
  return if (isGenericName()) block(this) else default
}

internal fun <T : Any> String.ifNotGenericName(
  default: T,
  block: (String) -> T = { default },
): T {
  return if (!isGenericName()) block(this) else default
