...
class TsPostScopeUseGenericInterceptor : TsPostScopeInterceptor() {
  ...
  ): Boolean = source.second.isRequireUseGeneric
  ...
  override fun process(
    ...
    return when (scope) {
      is TsScope.Interface -> {
        val r = scope.meta
        // TODO 处理父类类型
        val supers = ctx.getSuperTypeRefs(r)
        val properties =
          if (scope.properties.any { it.isRequireUseGeneric }) {
        ...
        scope.copy(superTypes = supers, properties = properties)
      }
