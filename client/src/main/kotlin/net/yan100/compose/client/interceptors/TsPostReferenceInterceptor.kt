...
/** 该拦截器内可以 获取到预处理好的 TypeVal */
abstract class TsPostReferenceInterceptor :
  Interceptor<ClientType, TsTypeVal<*>, KtToTsContext> {
  final override val executeStage: ExecuteStage =
    ExecuteStage.LOOP_RESOLVE_TS_REFERENCES

  override fun defaultProcess(
    ctx: KtToTsContext,
    source: ClientType,
  ): TsTypeVal<*> = TsTypeVal.Never
}
