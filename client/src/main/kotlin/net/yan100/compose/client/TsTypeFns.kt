...
fun TsVal.getUsedNames(): List<TsName> {
  return when (this) {
    ...
    is TsVal.Function ->
      params.flatMap { it.getUsedNames() } + returnType.getUsedNames()
  }
}
  ...
  return when (this) {
    ...
    is TsScope.TypeAlias ->
      aliasFor.getUsedNames() +
        name +
        usedGenerics
          .filterIsInstance<TsGeneric.Used>()
          .flatMap { it.used.getUsedNames() }
          .distinct()
...
fun TsTypeVal<*>.getUsedNames(): List<TsName> {
  return when (this) {
    ...
    is TsTypeVal.Function ->
      returns.getUsedNames() + params.flatMap { it.getUsedNames() }
    is TsTypeVal.Union -> joinTypes.flatMap { it.getUsedNames() }
    is TsTypeVal.TypeConstant -> element.getUsedNames()
    is TsTypeVal.Record ->
      keyUsedGeneric.getUsedNames() + valueUsedGeneric.getUsedNames()
    is TsTypeVal.Promise -> usedGeneric.getUsedNames()
    is TsTypeVal.Ref ->
      listOf(typeName) + usedGenerics.flatMap { it.getUsedNames() }
...
fun TsName.asTsImport(useType: Boolean = true): TsImport? {
  return when (this) {
    is TsName.PathName -> {
      TsImport(useType = useType, fromPath = path, usingNames = listOf(this))
    }
...
fun TsTypeVal<*>.asImports(useType: Boolean = true): List<TsImport> {
  return getUsedNames().mapNotNull { it.asTsImport(useType) }
}
...
fun TsScope<*>.collectImports(): List<TsImport> {
  val imports =
    when (this) {
      ...
      is TsScope.Interface -> {
        ...
        val properties =
          properties.flatMap {
            it.getUsedNames().mapNotNull { e -> e.asTsImport(true) }
          }
      ...
      is TsScope.Class -> {
        val functions =
          functions
            .flatMap { it.getUsedNames() }
            .mapNotNull { it.asTsImport(true) }
  ...
  return imports
    .mapNotNull { tsImport ->
      if (
        tsImport.fromPath == thisImport?.fromPath &&
      ...
      TsImport(
        ...
        fromPath =
          tsImport.fromPath.let { metaPath ->
            if (thisImport == null) {
              "../$metaPath"
            } else {
              ...
              check(thisPaths.size == 2) {
                "thisImport is not supported $thisPaths"
              }
              check(importPaths.size == 2) {
                "importPaths is not supported $importPaths"
              }
              val t1 = thisPaths[0]
    ...
    .groupBy { it.fromPath to it.useType }
    .map { (key, value) ->
...
fun List<TsImport>.toRenderCode(): String {
  ...
  return mapNotNull { tsImport ->
      ...
      val usingNames =
        tsImport.usingNames.joinToString(
          separator = ", ",
          prefix = "{",
          postfix = "}",
        ) { name ->
...
fun List<TsGeneric.Defined>.toRenderCode(): String =
