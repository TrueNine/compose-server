...
abstract class StubContext<C : StubContext<C>>(
  protected val interceptorChain: MutableList<Interceptor<*, *, *>> =
    mutableListOf()
) {
  inline fun <S : Any, T : Any, reified I : Interceptor<S, T, C>> List<I>
    .processOrNull(ctx: C, definition: S): T? {
    return filterIsInstance<I>()
      .firstOrNull { interceptor -> interceptor.supported(ctx, definition) }
