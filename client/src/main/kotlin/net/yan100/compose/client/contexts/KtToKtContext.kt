...
open class KtToKtContext(
  ...
  /** 已装载的服务端点 */
  val clientServiceMap =
    stub.services
      .groupBy({ it.toClientType() }) { it.operations }
          ...
          .map { operation ->
            val requestInfo =
              operation.requestInfo
                ?: error(
                  "RequestInfo is null class: ${clientType.typeName}#${operation.name}"
                )
            if (requestInfo.supportedMethods.size > 1) {
              requestInfo.supportedMethods.map { method ->
                operation.copy(
                  requestInfo =
                    requestInfo.copy(supportedMethods = listOf(method)),
  ...
  private val internalDefinitionsMap: MutableMap<String, ClientType> =
    mutableMapOf()
  ...
  private fun convertAllUsedGenerics(
    clientInputGenerics: List<ClientUsedGeneric>
  ...
  private fun convertAllPropertyName(
    clientProps: List<ClientProp>
  ): List<ClientProp> =
    clientProps.map {
      if (it.typeName.isGenericName()) return@map it
      it.copy(typeName = getTypeNameByName(it.typeName))
    }

  private fun dispatch(
    clientTypes: List<ClientType> = stub.definitions,
    deepCount: Int = 0,
  ): Map<String, ClientType> {
    if (deepCount > 16)
      error("Circular reference detected in updateDefinitions")
    if (clientTypes.isEmpty()) return emptyMap()
    val nameConvertedTypes =
      clientTypes.map { clientType ->
        clientType.copy(
          typeName = getTypeNameByName(clientType.typeName),
          superTypes = convertSuperTypes(clientType),
          aliasForTypeName =
            clientType.aliasForTypeName?.let { n -> getTypeNameByName(n) },
          usedGenerics = convertAllUsedGenerics(clientType.usedGenerics),
          properties = convertAllPropertyName(clientType.properties),
        )
      }
    if (!clientTypes.containsAll(nameConvertedTypes))
      internalDefinitionsMap.putAll(dispatch(nameConvertedTypes, deepCount + 1))
    internalDefinitionsMap.putAll(
      nameConvertedTypes.associateBy { it.typeName }
    )
  ...
  fun getTypeNameByName(typeName: String): String {
    if (typeNameToNameCache.containsKey(typeName))
      return typeNameToNameCache[typeName]!!
    if (
      internalDefinitionsMap.containsKey(typeName) &&
        internalDefinitionsMap.isNotEmpty()
    )
      return internalDefinitionsMap[typeName]!!.typeName
    val chains = interceptorChain.filterIsInstance<QualifierNameInterceptor>()
    if (chains.isEmpty())
      error(
        "Not a qualifier name interceptor was found, which may not have been expected"
      )
  ...
  fun getTypeByType(clientType: ClientType): ClientType? {
