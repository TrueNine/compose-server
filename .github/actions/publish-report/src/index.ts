...
export function generateSummaryReport(options: ReportOptions): string {
  ...
  if (publishSuccess) {
    ...
    if (primaryArtifact) {
      links.push(['Maven Central', `[Search](${getMavenSearchUrl(groupId, primaryArtifact)})`])
    }
    ...
    // Maven example
    const exampleArtifact = primaryArtifact ?? 'your-artifact'
    const mavenExample = generateMavenDependency(groupId, exampleArtifact, version)
    ...
    // Gradle example
    const gradleExample = generateGradleDependency(groupId, exampleArtifact, version)
...
async function run(): Promise<void> {
  try {
    ...
    // Set outputs
    // Truncate the report to keep GitHub Action outputs small
    setOutput('report', report.substring(0, 1000))
