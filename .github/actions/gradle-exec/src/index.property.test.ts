...
import { buildGradleCommand, buildGradleOpts } from './index.js'
...
describe('property 6: Gradle Execution Output Capture', () => {
  ...
  it('should include all tasks in the generated command', () => {
    const taskArb = fc.stringMatching(/^[a-z][a-z0-9:]*$/i)
  ...
  it('should always disable daemon in GRADLE_OPTS', () => {
    const optionsArb = fc.record({
      ...
      maxWorkers: fc.option(fc.integer({ min: 1, max: 16 })),
    })
  ...
  it('should generate valid command array for any option combination', () => {
    ...
    const optionsArb = fc.record({
      tasks: tasksArb,
      parallel: fc.option(fc.boolean()),
      buildCache: fc.option(fc.boolean()),
      configurationCache: fc.option(fc.boolean()),
      maxWorkers: fc.option(fc.integer({ min: 1, max: 16 })),
      timeout: fc.option(fc.integer({ min: 1, max: 120 })),
    })
