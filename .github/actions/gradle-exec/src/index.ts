...
export function buildGradleOpts(options: Partial<GradleExecOptions>): string {
  ...
  if (typeof options.maxWorkers === 'number' && options.maxWorkers > 0) {
...
export async function executeGradle(
  ...
  const timeoutMs = typeof options.timeout === 'number' ? options.timeout * 60 * 1000 : 30 * 60 * 1000
...
function parseInputOptions(): GradleExecOptions {
  ...
  return {
    ...
    maxWorkers: Number.parseInt(getInput('max-workers') || '4', 10),
    timeout: Number.parseInt(getInput('timeout') || '30', 10),
  }
}
