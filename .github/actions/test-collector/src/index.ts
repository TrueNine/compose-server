...
export function parseJUnitXml(content: string): JUnitTestSuite {
  ...
  const tests = Number.parseInt(extractAttribute(suiteTag, 'tests') || '0', 10)
  const failures = Number.parseInt(extractAttribute(suiteTag, 'failures') || '0', 10)
  const errors = Number.parseInt(extractAttribute(suiteTag, 'errors') || '0', 10)
  const skipped = Number.parseInt(extractAttribute(suiteTag, 'skipped') || '0', 10)
  const time = Number.parseFloat(extractAttribute(suiteTag, 'time') || '0')
  ...
  let selfClosingMatch = selfClosingRegex.exec(content)
  while (selfClosingMatch !== null) {
    const testCaseXml = selfClosingMatch[0]
    const testCase = parseTestCase(testCaseXml)
    testCases.push(testCase)
    selfClosingMatch = selfClosingRegex.exec(content)
  }
  ...
  let regularMatch = regularRegex.exec(content)
  while (regularMatch !== null) {
    const testCaseXml = regularMatch[0]
    const testCase = parseTestCase(testCaseXml)
    testCases.push(testCase)
    regularMatch = regularRegex.exec(content)
  }
 ...
 * @returns Attribute value or null when missing
 */
...
function parseTestCase(xml: string): JUnitTestCase {
  ...
  const testCase: JUnitTestCase = {
    ...
    time: Number.parseFloat(extractAttribute(tag, 'time') || '0'),
  }
...
async function run(): Promise<void> {
  try {
    ...
    // Truncate the summary to avoid exceeding GitHub output limits
    setOutput('summary', summary.substring(0, 1000))
